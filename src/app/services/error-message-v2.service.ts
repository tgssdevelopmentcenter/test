...
function ErrorMessageForControl(error: string, label: any): string {
  ...
  switch (error) {
    ...
    case "maxCash": errorMessage = `Cashback amount should be less than maximum amount!`;
      break
    case 'inValidDate': errorMessage = `From Date should be before To date!`;
      break
...
export class ErrorMessageV2Service {
  ...
  private forInvalidFormGroup(form: FormGroup): any {
    ...
    names.forEach((name: any) => {
      ...
      /**********************************For the Form array**************************************/
        ...
        let arr = []
        // console.log(control, 'in the form array---->')
  ...
  throwErrorMessage(form: AbstractControl, foundErrors: any[], labels: Labels[]) {
    ...
    switch (first.type) {
      ...
      case 'control': {
        ...
        this.toaster.showWarningToast(ErrorMessageForControl(firstControl?.errors, ctrLabel))
      }
        break;
      ...
      case 'array': {
        ...
        if (firstGroup?.group_errors != null) {
          this.toaster.showWarningToast(ErrorMessageForFormArray(firstGroup, firstControl, arrLabel, ''))
          return
        } else {
          const ctrLabel = findLabel(firstControl?.name, labels)
          ...
          this.toaster.showWarningToast(ErrorMessageForFormArray(firstGroup, firstControl, arrLabel, ctrLabel))
        }
      }
        break;
   ...
   *  let errorsV2 = this.errorMessageV2.invalidControls(this.personalizedForm);
    ...
    if (errorsV2.length > 0) {
      console.log(errorsV2, 'in the personalized component');
      this.errorMessageV2.throwErrorMessage(this.personalizedForm, errorsV2, labels)

    }
    return
    console.log(form, 'this is the value!')
   */
}
