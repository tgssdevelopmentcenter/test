...
export class SingleApiComponent implements OnInit, OnChanges {
      ...
      .subscribe((res: any) => {
        ...
        if (this.options?.length > 0 && this.options != undefined) {
          if (this.options_display != '' && this.options_value != '') {
            ...
            this.filteredOptions = this.options.filter((ele: any) =>
              String(ele).toLowerCase().includes(value.toLowerCase())
            );
          }
        }
      });
  ...
  optionSelected(event: any) {
    let data: any = this.options.filter((ele: any) =>
      checkNull(this.options_value)
        ? ele[this.options_value] == event?.value
        : ele == event?.value
    );
  ...
  getdata() {
    this.load = true;
    this.selectService
      .getOptions(this.api_endpoint, this.api_params, this.api_queryparams)
      .subscribe({
        next: (res: HttpResponse<any>) => {
          if (res?.statusText == 'OK') {
            this.load = false;
            this.options = checkNull(this.response_key)
              ? res?.body[this.response_key]
              : res?.body;
            this.filteredOptions = this.options;
          }
        },
